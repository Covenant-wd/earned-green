...
export default function AdminTasksPage() {
  ...
  const openCreate = () => {
    setEditingTask(null);
    setForm({ title: "", description: "", rewardAmount: "", platform: "", link: "", category: "", difficulty: "Easy" });
    setDialogOpen(true);
  };

  const openEdit = (task: typeof mockTasks[0]) => {
    setEditingTask(task);
    setForm({ title: task.title, description: task.description, rewardAmount: String(task.rewardAmount), platform: task.platform, link: task.link, category: task.category, difficulty: task.difficulty });
    setDialogOpen(true);
  };
  ...
  const handleSave = () => {
    ...
    if (editingTask) {
      setTasks((prev) => prev.map((t) => t.id === editingTask.id ? { ...t, ...form, rewardAmount: parseFloat(form.rewardAmount) } : t));
      toast.success("Task updated");
    } else {
      setTasks((prev) => [...prev, { ...form, id: String(Date.now()), rewardAmount: parseFloat(form.rewardAmount), type: "social", isActive: true, maxCompletions: 100, status: "active" }]);
    ...
    <div className="page-container">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="glass-card border-border">
          <DialogHeader>
            <DialogTitle className="font-display">{editingTask ? "Edit Task" : "Create Task"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
